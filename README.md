...
- **T13 執行中**：成績中心新增「成績列印」頁籤（多人勾選、列印前選項、預覽後下載 PDF）。
- **報到總覽**：改為僅顯示進行中訓練計畫；報到統計 Modal 改為統計卡片切換單一清單（應到/實到/未到/請假），並支援列印目前清單。
- **考卷工坊/題庫維護**：新增題目全選/不全選與批次刪除。
- **考試中心**：未報到時按「開始考試」會先顯示提示，提供「立即報到」再進入考試。
