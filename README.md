...
| **訓練計畫** | 年度計畫、受課單位／個人授課對象、**免考角色／單位／個人**（開課單位旁「免考設定」；超管預設鎖定免考且免報到，其餘免考者仍須報到）、封存、頁籤（進行中／已過期／已封存）、篩選與排序。 |
...
| **系統管理** | 單位、分類、**教材主檔**（類型／允許副檔名）、人員、角色、權限、功能清單、職務；**排程備份**（NAS 連線測試、立即備份、備份紀錄分頁與可選刪 NAS 之批次刪除；權限碼 `menu:admin:backup`）。 |
...
- [專案系統架構分析](1.docs/00-專案總覽/專案系統架構分析.md) — 系統架構、流程、資料庫、技術棧、前後端每支檔案用途  
...
完整慣例、驗證指令與錯誤對照見 [NAS與路徑跨平台慣例](1.docs/00-專案總覽/NAS與路徑跨平台慣例.md)；**Docker 部署總覽**見 [生產部署指南](1.docs/00-專案總覽/生產部署指南.md)；SMTP 加密步驟見 [MIGRATION_GUIDE](1.docs/00-專案總覽/資料庫遷移/MIGRATION_GUIDE.md)。
...
**最後更新**：2026-08-19（架構 SSOT 補程式檔案用途）
...
- 擴充 [專案系統架構分析](1.docs/00-專案總覽/專案系統架構分析.md) 為現行架構 SSOT：整體／頁面↔API 圖、後端與前端**每支檔案用途**。
