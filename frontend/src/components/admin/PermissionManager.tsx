...
const HIDDEN_FUNCTION_CODES = new Set(['menu:admin:func']);

function filterFunctions(funcs: SystemFunction[]): SystemFunction[] {
  return funcs
    .filter((f) => !HIDDEN_FUNCTION_CODES.has(f.code))
    .map((f) => ({
      ...f,
      children: f.children?.length ? filterFunctions(f.children) : [],
    }));
}
...
const PermissionManager = () => {
  ...
  const isRoleProtected = selectedRole ? isProtectedSystemRole(selectedRole.name) : false;
  ...
  const fetchInitialData = async () => {
    try {
      ...
      setFunctions(filterFunctions(funcsRes.data));
  ...
  const renderFunctionItem = (func: SystemFunction) => {
    const isChecked = rolePermissions.has(func.id);
    const isDisabled = isRoleProtected;
    ...
    return (
      ...
      >
        <div
          className={`w-5 h-5 shrink-0 rounded-md border-2 flex items-center justify-center transition-colors duration-200 ${
        ...
        <div className="flex-1 min-w-0">
          <div className="font-bold text-gray-800 truncate">{func.name}</div>
          <div className="text-xs text-indigo-400 font-mono truncate">{func.code}</div>
        </div>
      </label>
    );
  };

  const renderFunction = (func: SystemFunction) => {
    const isAdminParent = func.code === 'menu:admin';
      ...
      <div key={func.id} className="mb-2">
        ...
        {hasChildren && (
          <div
            className={
              isAdminParent
                ? 'mt-2 pl-4 border-l-2 border-indigo-100 ml-5 grid grid-cols-1 sm:grid-cols-2 gap-2'
                : 'mt-1 pl-4 border-l-2 border-indigo-100 ml-5 space-y-1'
            }
          >
            {func.children.map((child) =>
              isAdminParent ? renderFunctionItem(child) : renderFunction(child),
            )}
          </div>
        )}
      </div>
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
              ...
              {roles.map((role, index) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-black text-indigo-300 w-6">
                      {(index + 1).toString().padStart(2, '0')}
                    </span>
                    <div className="flex items-center gap-2">
                      <Shield
                        className={`w-4 h-4 ${selectedRoleId === role.id ? 'fill-current' : 'text-indigo-400'}`}
                      />
                      {role.name}
                    </div>
                  </div>
        ...
        <div className="md:col-span-8 bg-white rounded-2xl shadow-sm border border-indigo-100/50 overflow-hidden flex flex-col">
          <div className="p-4 bg-linear-to-r from-indigo-50/50 to-purple-50/30 border-b border-indigo-100 flex items-center justify-between sticky top-0 z-10 gap-3">
            <div className="font-black text-indigo-500 uppercase tracking-wider text-xs">
              功能存取權限
            </div>
            <button
              onClick={handleSave}
              disabled={isSaving || !selectedRoleId || isRoleProtected || !isDirty}
              className="bg-green-500 text-white px-4 py-2.5 rounded-xl font-bold text-sm shadow-md shadow-green-200 hover:bg-green-600 hover:shadow-lg active:scale-95 transition-all duration-200 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shrink-0"
            >
          ...
          <div className="p-6 h-[900px] overflow-y-auto">
            {isRoleProtected && (
              <div className="mb-4 p-3 bg-amber-50 text-amber-800 rounded-xl text-sm font-medium border border-amber-100">
                系統內建角色（{selectedRole?.name}）的權限由系統固定，無法變更。
              </div>
            )}
            {error && (
            ...
            {functions.length === 0 && !isLoading ? (
              ...
              <div className="space-y-1">{functions.map((func) => renderFunction(func))}</div>
            )}
          </div>
        </div>
      </div>
