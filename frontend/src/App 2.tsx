...
const Navbar = ({ user, onLogout }: { user: User; onLogout: () => void }) => {
  ...
  const navItems = [
    { name: '首頁', path: '/', icon: <LayoutDashboard className="w-4 h-4" />, code: 'menu:home' },
        ...
        {!user ? (
          ...
          <>
            ...
            <main className="max-w-7xl mx-auto px-4 md:px-6">
              <Routes>
                <Route path="/" element={<PagePlaceholder title="首頁 / 個人考試中心" />} />
                <Route path="/plans" element={user.functions?.includes('menu:plan') || user.role === 'Admin' ? <TrainingPlanManager /> : <Navigate to="/" />} />
                <Route path="/exams" element={user.role === 'Admin' ? <PagePlaceholder title="考卷工坊" /> : <Navigate to="/" />} />
                <Route path="/reports" element={<PagePlaceholder title="成績中心" />} />
