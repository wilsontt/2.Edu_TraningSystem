...
**文件類型**：棕地實作計劃  
**建立日期**：2026-07-20  
**狀態**：✅ 已實作（分支 `cursor/batch-checkin-history-3b7d`）；人工驗收待勾選後可結案並移入 `plans/已完成/`  
...
1. **依序多場、一次報到**：同一日上午依序進行之多場訓練（如工安 → 個資 → 資安），學員僅需掃描**一組合併 QR**，系統為各計畫各寫一筆有效報到（`attendance_records`），`checkin_time` 相同。
...
| # | 決策 |
|---|------|
| Q1 | 勾選條件：**≥2 場**、**正在進行中**（`status=active`）、**同一 `training_date`**（預設以「今日」篩選） |
...
| 單計畫報到 QR | 既有 `/checkin?plan_id=<id>`；用於遲到、請假、補課、漏掃 |
...
| `training_date` | Date | 勾選計畫之基準日（冗余便於查詢） |
...
| Method | Path | 說明 | 權限 |
|--------|------|------|------|
| `POST` | `/training/attendance/batches` | body: `{ plan_ids[], label? }`；驗證 ≥2、同日、active；回 batch + QR | `menu:attendance-overview` |
...
| # | 情境 | 預期 | 狀態 |
|---|------|------|------|
| T1 | 勾選 3 場同日 active ≥2 | 可建立批次並顯示大 Modal QR | ☐ |
