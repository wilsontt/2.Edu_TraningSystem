...
**建立日期**：2026-06-18
**狀態**：✅ 已完成（Wave 0～6 程式與 security-review 已完成；**產品驗收 2026-07-14**）
**規格準據**：[`20260618_教育訓練_建議事項_PLAN.md`](./20260618_教育訓練_建議事項_PLAN.md)

> 本文件將定案規格拆解為 **Wave 0～6** 之可執行步驟、程式落點、驗收與排程。
>
> **2026-07-14**：產品實測確認業務情境無問題；各 Wave「UAT 另追」改為已驗收。特殊環境項（AD 人工步驟、批次列印 Docker CJK）仍見 [棕地功能總覽 §4](../../棕地功能總覽.md)。
>
> **關聯 PLAN**：
...
- [x] 手動驗證無需 F5（**產品驗收 2026-07-14**：考試中心 → 成績中心 → 返回 → 按鈕可點）

> **實作摘要（2026-06-22）**：前端 `frontend/src/components/exam/ExamDashboard.tsx`、`CheckInButton.tsx`；後端無變更。已通過 `npm run lint`（0 問題）與 `npm run build`（tsc + vite 成功）。**產品驗收 2026-07-14**。
...
- [x] 手動驗證（**產品驗收 2026-07-14**：建立/編輯計畫 UX、儲存重開一致、跨單位個人於考試中心可見並可報到應考）
...
- [x] 整合驗證（**產品驗收 2026-07-14**：SMB 環境上傳落 NAS `exams/`、audit、斷線行為已實測）
...
- [x] NAS 登入 + 傳輸 UI（前端完成）：`NasLoginModal`（驗證取 token）＋ `FileTransferModal`（進度 %、取消 AbortController、beforeunload、僅 X／取消可關、**禁點遮罩關閉**、成功自動關閉）
- [x] **前端**：`teaching/NasLoginModal.tsx`、`teaching/FileTransferModal.tsx`、`teaching/PlanMaterialsSection.tsx`（計畫編輯頁教材區：上傳＋同名衝突二選一對話框＋列表＋下載＋軟刪）、`teaching/TeachingMaterialLibrary.tsx`（搜尋／類型/格式篩選、跨頁勾選、批次下載確認、單檔下載、軟刪、分頁）、`App.tsx` 路由 `/teaching-materials` ＋導覽列（code `menu:exam`）、`TrainingPlanManager` 內嵌教材區
- [x] **教材類型維護 UI**（已由 [`20260704_教材主檔與允許格式維護_PLAN.md`](./20260704_教材主檔與允許格式維護_PLAN.md) 完成：`MaterialMastersManager.tsx`）
- [x] 整合驗證（**產品驗收 2026-07-14**：SMB 上傳/衝突/取消/下載/批次 ZIP 已實測）

> **前端實作摘要（2026-06-22）**：新增 `frontend/src/components/teaching/`（transfer.ts、NasLoginModal、FileTransferModal、PlanMaterialsSection、TeachingMaterialLibrary）；`App.tsx` 加教材庫路由與導覽；`TrainingPlanManager` 編輯既有計畫時內嵌教材區。下載/批次以 blob + `onDownloadProgress`、上傳以 FormData + `onUploadProgress`，取消用 `AbortController`。每次傳輸前 NAS 登入（符合教材 PLAN §5.4a）。`npm run lint` 0 問題、`npm run build` 成功。
> **限制**：① 教材類型維護 UI 未做（API 已就緒）；② 取消傳輸的 `cancelled` 後端稽核未串（前端 abort 後端僅記 failed/無記錄）——可後續加一個輕量 cancel 回報端點。
...
- [x] 資料表與加密（`BackupScheduleConfig` 單例 id=1、`BackupRecord`；密碼以 Fernet 加密，金鑰 `BACKUP_CREDENTIAL_SECRET` 環境變數，不存明文）
- [x] `backend/app/services/backup_service.py`：`sqlite3 .backup` API 一致性備份 → ZIP（含 `manifest.json`）→ 上傳 NAS → 依保留份數清除最舊
- [x] `backend/app/services/scheduler.py`：APScheduler `BackgroundScheduler`，設定變更即時 reschedule（不需重啟）
- [x] `routers/backup.py`：`GET/PUT /admin/backup/config`、`POST /admin/backup/run-now`、`GET /admin/backup/records`；獨立權限代碼 `menu:admin:backup`（可於權限管理單獨指派）
- [x] 前端 `BackupScheduleManager.tsx`：排程設定表單、立即備份、備份紀錄（Wave 4 當時使用 `@shared-ui/data-table` 分頁）
- [x] 實機驗證（使用者已實際設定排程帳密並執行過數次成功備份）

> **後續補強（2026-08-07，非本 Wave 原範圍）**：NAS 連線測試、紀錄項次／`common/Pagination`（對齊人員管理）、可選刪 NAS 之批次刪除。見 [`../../reviews/20260807_排程備份_連線測試與紀錄批次刪除.md`](../../reviews/20260807_排程備份_連線測試與紀錄批次刪除.md)。

> **範圍調整（使用者決策，2026-06-23）**：原規格「DB＋當日 materials 快照」改為**僅備份 DB 本體**——教材／考卷實體檔已存於 NAS，由 NAS 端既有 3-2-1 備援機制保障，重複打包 NAS 上的整顆 materials 樹（下載→重新壓縮→再上傳回 NAS）成本高且無實際 DR 增益。
> **實作摘要**：新增 `crypto.py`（Fernet 加解密）、`scheduler.py`、`backup_service.py`、`routers/backup.py`；`storage.py` 補 `backup_credentials()`；遷移 `add_backup_schedule.py`、`add_backup_permission_and_cleanup.py`（後者同時清除除錯腳本 `debug_api_cors.py` 遺留的 `TestScriptRoot` 權限項，並把新權限授予既有持有 `menu:admin` 的角色）。
> **教材庫位置調整**：教材庫頁面由獨立路由併入「考卷工坊」頁籤（按鈕切換，沿用 `menu:exam` 權限），系統管理選單與權限管理皆不需為此新增項目。

---

### Wave 5 — 行動 UI（#4）

- `ExamDashboard.tsx`、`CheckInButton.tsx`、成績中心（`PersonalScorePage`/`PersonalScoreOverview`/`PersonalScoreHistory`/`PersonalLearningAnalysis`/`ScoreDetailModal`）：responsive、safe-area、觸控目標。
- 完成後撰寫 skill（`mobile-responsive-training-ui`，存於專案 `.claude/skills/`）。
...
- [x] `frontend/index.html` 加 `viewport-fit=cover`（啟用 `env(safe-area-inset-*)`）
- [x] 考試中心：卡片/標頭 padding 響應式、報到按鈕 `w-full` + 觸控目標 ≥44px、開放時間/挑戰次數行 `flex-wrap`
- [x] 成績中心：4 頁籤 Tab bar 改 `overflow-x-auto` 橫向可滑動（原 `w-fit` 在窄螢幕會擠壓/溢出）、KPI／計畫選擇器 grid 補 `grid-cols-1`、趨勢圖頁籤切換列窄螢幕改堆疊
- [x] `ScoreDetailModal.tsx` 資訊欄位 `grid-cols-2` → `grid-cols-1 sm:grid-cols-2`
- [x] 僅調整 layout，未變更商業邏輯與 API（`npm run lint`、`npm run build`、`tsc` 皆通過）
- [x] 瀏覽器實測：以 Playwright（headless Chromium）於 375×667／390×844 viewport 對已登入頁面截圖驗證，含「未報到→立即報到」彈窗互動，無 console 錯誤
- [x] iOS／Android 實機驗證（Playwright 模擬器已驗；**產品驗收 2026-07-14**）

> **範圍邊界**：未調整 `@shared-ui/portal-nav`（頂部導覽列，跨專案共用元件，已有自身 `flex-wrap` 與漢堡選單邏輯，不屬本專案調整範圍）；未將既有 `<table>`（成績歷史列表）改為卡片式版面（維持 `overflow-x-auto` 橫向滾動，表格轉卡片屬更大改版，列為未來可選項）；`ReportDashboard.tsx`（部門成績／管理端報表）依規格屬第二階段，本波未動。
...
- [x] security-review 報告已產出（`reviews/20260618_建議事項_security-review.md`）
