...
**建立日期**：2026-06-18
**狀態**：🟡 Wave 0～4 主體已完成；Wave 5（行動 UI）／Wave 6（security-review）待整波結案
...
- [ ] **教材類型維護 UI 延後**（類型已由 init_db/migration 植入 7 種、`material-types` CRUD API 已存在；管理頁待後續或併入系統管理）
- [ ] 整合驗證（需 `backend/.env` SMB + `pip install smbprotocol`；實機測上傳/衝突/取消/下載/批次 ZIP）
...
- [ ] iOS／Android 實機驗證（待使用者於真實裝置確認，模擬器截圖已確認版面正確）
