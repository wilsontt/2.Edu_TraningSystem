...
**建立日期**：2026-06-18
**狀態**：✅ 已實作（Wave 0～6 程式與 security-review 已完成；部分手動／實機驗證為 UAT 另追）
...
- [x] 手動驗證無需 F5（程式完成；UAT 另追：考試中心 → 成績中心 → 返回 → 按鈕可點）
...
- [x] 手動驗證（程式完成；UAT 另追：建立/編輯計畫 UX、儲存重開一致、跨單位個人於考試中心可見並可報到應考）
...
- [x] 整合驗證（程式完成；UAT 另追：需 SMB 環境實測上傳落 NAS `exams/`、audit 有紀錄、斷線回 503）
...
- [x] 整合驗證（程式完成；UAT 另追：需 SMB 環境實測上傳/衝突/取消/下載/批次 ZIP）
...
- [x] iOS／Android 實機驗證（程式完成；Playwright 模擬器已驗版面；真機 UAT 另追）
