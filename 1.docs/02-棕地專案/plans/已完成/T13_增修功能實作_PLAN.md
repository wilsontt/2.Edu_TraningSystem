...
> **狀態補註（2026-05-02）**：本檔為 T13 增修之**歷史實作計劃**；上列 YAML `todos` 與下方 Checklist 已標為**完成**，與 `1.docs/交付實作文件/README.md` 及 2026-04～05 各期交付文件、程式現況對齊。後續若僅調整細節，請改寫交付實作文件或 PR 說明，**勿**再依本檔未勾選項判斷為未完成。

## 執行與驗收 Checklist

> ~~規則：只有「你測試確認無誤」後，該項目才可從進行中改為完成。~~（本輪已結案，保留原文供追溯。）

- [x] T13-1 建立並套用 Hybrid 可視範圍 resolver（role > job_title > self）至報到/成績/列印查詢
- [x] T13-2 完成成績中心列印問答流程與成績列印頁籤（清單/個人成績、多計畫選擇）
- [x] T13-3 補齊列印清單能力：序號、分頁、排序、關鍵字查詢、標題與4卡統計
- [x] T13-4 修正報到總覽：批次未報到原因、選中卡片可視化、即時重算統計
- [x] T13-5 確保報到總覽與訓練計畫報到統計口徑一致並完成差異修正
- [x] T13-6 完成考卷工坊增強：上傳區左右版型、搜尋清除X、全選/不全選、多標籤編輯
- [x] T13-7 完成考試中心與訓練計畫補強：僅進行中且本人可見、隱藏未啟用人員
- [x] T13-8 執行角色情境與列印流程的回歸測試並整理驗收結果

# T13 增修功能實作 PLAN（測試問題＋新增需求）

## 目標與範圍

完成 `1.docs/02-棕地專案/規格與計畫/T13 增修功能實作PLAN_測試問題.md` 的全部項目，涵蓋：
