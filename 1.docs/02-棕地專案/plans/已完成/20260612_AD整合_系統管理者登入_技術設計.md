...
**文件類型**：技術詳細設計（TDD）
**建立日期**：2026-06-25
**狀態**：✅ 已實作（2026-06-29 驗收；待人工測試項目見 `reviews/20260629_AD整合_驗收記錄.md`）
...
| `AD_SERVER_URI` | str | "" | DC 連線位址（IP 或主機名稱）；屬基礎設施資訊，僅存 `.env`，`.env.example` 用假值 |
| `AD_USE_SSL` | bool | false | false = LDAP（port 389）；true = LDAPS（port 636，需 DC 有效憑證） |
| `AD_BASE_DN` | str | "" | |
| `AD_DOMAIN` | str | "" | UPN 網域 |
| `AD_ADMIN_GROUP` | str | IT Admins | |
...
| `SMTP_PORT` | int | 587 | |
| `SMTP_USER` | str | "" | |
| `SMTP_PASSWORD` | str | "" | 機敏，**僅存 `.env`，不入版控**；建議 `enc:<Fernet密文>`（`scripts/encrypt_env_secret.py`），金鑰為 `CREDENTIAL_SECRET`（或相容 `BACKUP_CREDENTIAL_SECRET`）；明文仍相容但啟動警告 |
| `CREDENTIAL_SECRET` | str | "" | Fernet 金鑰（SMTP 密文等）；未設則沿用 `BACKUP_CREDENTIAL_SECRET` |
| `SMTP_FROM` | str | "" | 寄件者 |
...
| 項目 | 規範 |
|------|------|
| 版控 | `backend/.env` 列入 `.gitignore`，**禁止 commit** |
| 範本 | `backend/.env.example` 可版控，但只放**假值／空白**；不得含任何真實 IP、帳密、金鑰 |
| 檔案權限 | 伺服器上執行：`chmod 600 backend/.env`，並確認 owner 為執行程式的 OS 使用者 |
| 機敏欄位 | `JWT_SECRET_KEY`、`CREDENTIAL_SECRET`／`BACKUP_CREDENTIAL_SECRET` — 金鑰明碼存於 `.env`（OS 檔案權限保護）；`SMTP_PASSWORD` 建議存 `enc:` Fernet 密文（執行期解密）。更高合規需求請改用金鑰管理服務（HashiCorp Vault 等） |
| 基礎設施資訊 | `AD_SERVER_URI`（DC IP）、`SMB_SERVER` 等雖非憑證，仍屬機敏，只存 `.env`，不入版控 |
| 上線前確認 | `JWT_SECRET_KEY` 已換強密鑰；`LOGIN_BYPASS_ENABLED=false`；`AD_FALLBACK_EMAIL_ENABLED` 依需求設定 |
...
AD_USERNAME_PATTERN = re.compile(r"^[a-zA-Z0-9][a-zA-Z0-9._-]{0,63}$")  # 驗證 sAMAccountName（最長 64 字元）

def extract_sam_account(raw: str) -> str: ...   # 從三種格式提取 sAMAccountName
def normalize_ad_username(raw: str) -> str: ... # extract + 轉小寫
...
**break-glass 初始密碼**（環境變數 `INITIAL_ADMIN_PASSWORD`）：

- 遷移腳本須以 **`backend/.venv/bin/python3`** 執行（不可用系統 Python）。
- 雜湊使用 **`bcrypt` 套件直接**（與 `auth_utils.hash_password` 一致）；`passlib` 僅用於 Email OTP（`sha256_crypt`）。
- 未寫入 `password_hash` 時腳本須 **exit 1**（不可僅 warning 後完成）。
- 排查步驟見 `1.docs/02-棕地專案/交付實作文件/20260625_AD整合_系統管理者登入-CloudCode提示詞.md` §10。
...
- 掛 `AD_ADMIN_ROLE_NAME`、預設部門（預設 `系統管理`，避免與實體 IT 部門混淆）
...
def is_ad_unreachable() -> bool:
    """輕量 TCP/LDAP 探測或最近一次 AD 登入失敗快取；用於決定是否允許路徑 D。"""
...
| username 驗證 | 支援三種格式（`username`、`user@domain.com`、`DOMAIN\username`）；schema validator 提取 sAMAccountName 後驗證 `AD_USERNAME_PATTERN`；Pydantic `max_length=128` |
...
2. **AD 管理登入**：`username` + `password`
   - 前端 regex 支援三種 AD 格式；後端 validator 提取 sAMAccountName 再比對 `AD_USERNAME_PATTERN`
   - `maxLength={128}`；`autoComplete="username"`
...
1. LDAP（含可選 LDAPS）／密碼不落地／JWT 環境變數
