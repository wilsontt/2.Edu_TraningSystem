...
| `SMTP_PORT` | int | 587 | |
| `SMTP_USER` | str | "" | |
| `SMTP_PASSWORD` | str | "" | 機敏，**僅存 `.env`，不入版控**；`.env` 須設 `chmod 600` 保護 |
| `SMTP_FROM` | str | "" | 寄件者 |
...
| 機敏欄位 | `JWT_SECRET_KEY`、`SMTP_PASSWORD`、`BACKUP_CREDENTIAL_SECRET` — 明碼存於 `.env`，由 OS 檔案權限保護；若公司有更高合規需求，改用金鑰管理服務（HashiCorp Vault 等） |
