...
**建立日期**：2026-07-17  
**最近修訂**：2026-07-20  
**狀態**：✅ 已實作（分支 `feature/checkin-owner-scope-20260717`，commit `f21478b`）；瀏覽器手動驗收項待勾選後可結案並移入 `plans/已完成/`  
...
4. **考卷工坊 NAS 韌性**：`list_materials` 在 NAS 不可達時仍回 200 空陣列，不中斷題目讀取與管理（上傳／刪除檔仍 503）。
...
| **E 考卷 NAS** | `GET /admin/exams/materials/{plan_id}` NAS 不可達 → 200 `[]` + `X-NAS-Unavailable`；CORS 暴露標頭；前端琥珀色提示 |
...
| 報到未到原因／列印清冊 Owner | 有 `menu:plan` 或報到總覽權限即可，不套用開課單位 |
...
| Q6 | Owner **寫入**＝編輯＋刪除（非僅刪除）；訓練計畫管理含封存／取消封存／產生 QR（**報到總覽** QR 不綁 Owner → 見 [20260720 PLAN](20260720_合併報到批次與報到歷程_PLAN.md)） |
| Q7 | 考卷工坊考題 Owner 依 **`TrainingPlan.dept_id`**（`Question` 無獨立 `dept_id`）；非 Owner 檢視模式 |
...
| 審核者 | 核可範圍決策；合併前完成 T4-2～T4-4／T4-6 瀏覽器驗收 |
...
| 名詞 | 說明 |
|------|------|
| 開課單位 / Owner | `TrainingPlan.dept_id`；題庫／套組寫入對應 `dept_id`；考卷工坊考題以所屬計畫之 `dept_id` 判斷 |
| 寫入權 | `can_modify_owned_resource()`；含編輯、刪除、封存、上傳、匯入等變更 API |
...
| 檢視模式 | 前端 `isViewOnly`／`isOwnerReadOnly`：非 Owner 可讀列表／預覽，變更按鈕 disabled |
...
| 項目 | 落點 |
|------|------|
| 產生 QR | `POST /training/plans/{id}/checkin-qrcode/generate`（Owner）；`TrainingPlanManager`、`AttendanceOverviewPage` |
...
| 資源 | 後端（403 非 Owner） | 前端（非 Owner） |
|------|----------------------|------------------|
| **訓練計畫** | `PUT`、刪除、封存／取消封存、產生報到 QR | `TrainingPlanManager` 檢視模式、`fieldset disabled` |
| **歷史題庫** | `PUT`、刪除／批次刪除 | `QuestionBankManager` 編輯／刪除 disabled |
| **教材套組／檔** | `PUT`、綁定計畫、上傳檔、刪除 | `TeachingMaterialLibrary`、`MaterialSetEditPanel`、`PlanMaterialsSection` |
| **考卷工坊考題** | `POST /upload`、`/import-from-preview`、`PUT`／`DELETE` questions、`DELETE` materials、`POST /question-bank/import` | `ExamStudio` 檢視模式、`isWriteLocked` |

| 共用 | 落點 |
|------|------|
| Migration | `add_owner_dept_fields.py` |
| 判斷 | `access_scope.can_modify_owned_resource`（`can_delete_owned_resource` 別名）；`authGuards.canModifyOwnedResource` |
| 篩選 | 題庫／教材列表 `dept_id` 查詢參數 |
| 寫入 dept_id | 考卷上傳寫題庫帶 `plan.dept_id`；新增套組必填 `dept_id` |
...
| 項目 | 落點 |
|------|------|
| 後端 | `exam.list_materials`：`StorageUnavailable` → 200 `[]` + header `X-NAS-Unavailable: 1` |
| CORS | `main.py` `expose_headers` |
| 前端 | `ExamStudio` 琥珀色 `materialsNasWarning`；題目區不受影響 |
| 測試 | `tests/test_exam_list_materials_nas.py` |
...
| 使用說明 | `1.docs/00-專案總覽/專案使用說明.md` §3.2／§3.3／§3.6 |
| 遷移指南 | `1.docs/00-專案總覽/資料庫遷移/MIGRATION_GUIDE.md`（Owner、attendance UNIQUE） |
| 部署故障 | `1.docs/00-專案總覽/生產部署指南.md` §5.7 |
...
| T5 | 非 Owner 編輯／刪除／封存（計畫／題庫／教材） | 前端 disable／檢視模式＋後端 403 | 單元測試已過；待人工 |
| T6 | 超管編輯／刪除 | 允許 | 單元測試已過；待人工 |
| T7 | 教材編輯鎖／檔案刪 | 符合 §5.3 | 待人工 |
| T8 | 統計 Modal QR 版面 | 左 QR 大、右四卡小 | 程式已修；待人工 |
| T9 | pytest／lint／build | 通過 | ✅ |
| T10 | 非 Owner 考卷工坊 | 可讀題目／預覽 TXT；不可上傳／匯入／編輯／刪除考題 | 單元測試已過；待人工 |
| T11 | NAS 不可達 list_materials | 200 空陣列＋琥珀色提示；題目仍可載入 | 單元測試已過；待人工 |
...
**2026-07-20 定案補充**：Owner 範圍擴為**寫入權**（含編輯，非僅刪除）；考卷工坊所選計畫之考題亦納入；非開課單位僅檢視／下載。
