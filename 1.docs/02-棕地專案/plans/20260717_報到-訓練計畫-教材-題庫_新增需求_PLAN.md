...
2. **開課單位＝Owner**：訓練計畫、歷史題庫、教材套組／檔案僅開課單位（或超管）可刪，其餘僅能檢視／下載。
...
| **B Owner** | `question_bank.dept_id`、`teaching_material_sets.dept_id`；刪除權限；列表／搜尋開課單位 |
...
| 判斷 | `access_scope.can_delete_owned_resource`；前端 `authGuards.canDeleteOwnedResource` |
| 刪除／篩選 | `training.delete_plan`、`question_bank`、`teaching_material_sets` |
...
| T5 | 非 Owner 刪除 | 前端 disable＋後端 403 | 單元測試已過；待人工 |
| T6 | 超管刪除 | 允許 | 單元測試已過；待人工 |
