...
> **狀態補註（2026-05-01）**：與「成績中心 — 部門績效表現、部門成員批次列印（list／individual 對齊個人預覽路徑）、展開成員成績列表欄寬與版面」相關之待測與修正項，已於程式與 [交付實作文件](../交付實作文件/README.md) 追溯完成。本檔其餘章節仍保留為規格與追蹤脈絡；文中標為選配或未來擴充者不在此補註範圍內。
...
1. 列印每個人的考卷成績：
a. 沒有隔列Highlight顯示。
b. @1.docs/02-棕地專案/規格與計畫/T13 增修功能實作PLAN_測試問題.md:48-53 ，都沒做到。
1. 
...
> **文件補註（2026-05-02）**：早期條列曾寫「只顯示進行中」再「續 1 加篩選」，易讀成與下方結案註記矛盾。**以下「最終規格」為準**；上兩句原始需求僅供追溯討論過程。

**最終規格（定案，與訓練計畫管理一致）**

1. **個人成績總覽**與**部門成績／成績中心統計報表**：使用者可於 **進行中／已過期／已封存** 三態間切換（預設 **進行中**），前後端以 `plan_status`（`active`／`expired`／`archived`）對齊；**不得**再解讀為「畫面永遠只能看到進行中且無法切換」。
2. UI 文案與 query 語意須與訓練計畫管理頁籤一致（實作落點見結案註記）。
...
> **文件補註（2026-05-02）**：下一句「尚需修正」為**修正當下**之逐字紀錄；緊接之子項均已標 **（已修正）**，請以子項為準，勿再依該句判斷為未完成。
...
> **文件補註（2026-05-02）**：本條為**學員端「我的考試」列表**之原始需求敘述，與上文 **§成績中心**（管理端報表 `plan_status` 三態切換）**不同模組**。實作上：**已封存**計畫於 `GET .../my_exams` 所依底層 query 已排除（`is_archived` 為 false／null）；**已過期**是否仍出現於列表、或僅以狀態標示為不可考，以現行 `backend/app/routers/exam_center.py` 及前端考試中心為準。請勿將本條與管理端「進行中／已過期／已封存」頁籤混讀為同一規格。
