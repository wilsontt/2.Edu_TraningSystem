...
**建立日期**: 2026-01-13  
**狀態**: ✅ 已完成（2026-05-02 文件同步）

> **結案補註**：`login_with_qrcode` 已於 `backend/app/routers/auth.py` 使用 `joinedload` 預載入（對應原「問題3」）。`QRCodeManager.tsx` 已實作僅保留最近 5 筆已過期 Token 等邏輯；介面未再使用 `used_at`／「使用時間」欄。原「問題4」時區顯示若仍有邊界案例，請另開單追蹤。內文若引用舊行號，以目前檔案為準。
