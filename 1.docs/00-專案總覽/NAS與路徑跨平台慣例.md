...
| Linux Docker | compose／`environment` 注入同名變數；映像**不含** `.env` |
...
| 症狀 | 原因 | 處理 |
|------|------|------|
| `NAS 共享尚未設定（需 SMB_SERVER／SMB_SHARE）` | 執行中後端未讀到非空設定 | 檢查 `.env` 或 compose 環境變數後完整重啟 |
| 設定有值仍失敗 | OS 環境變數以空字串覆寫 `.env` | 清除空的 `SMB_*` 環境變數 |
| 連線失敗（非「尚未設定」） | 網路、帳密、共享權限 | 本機驗證可存取 `\\{SMB_SERVER}\{SMB_SHARE}` |
