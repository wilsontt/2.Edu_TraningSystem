...
| 教材庫 | 教材上傳/下載、NAS 登入 session、類型與格式主檔 | `/admin/teaching-materials` |
...
【教材上傳】（與考卷分離）
訓練計畫編輯頁或教材庫 → NAS interactive 登入 → 上傳至 NAS → 寫入 teaching_materials
...
- **教材（棕地）**：`material_types`／`material_file_formats` 主檔 → `teaching_materials`（實體檔於 NAS）。
...
| **teaching_materials** | 教材目錄卡 | id, plan_id, title, material_type_id, storage_path, is_active |
...
- **API 與模組細節**：[02-棕地專案/規格與計畫/0.專案架構分析.md](../02-棕地專案/規格與計畫/0.專案架構分析.md)  
...
**文件版本**：v1.0.0  
**最後修訂**：依專案實際更新  
**維護**：架構或技術變更時請同步更新本文件與 [02-棕地專案/規格與計畫/0.專案架構分析.md](../02-棕地專案/規格與計畫/0.專案架構分析.md)。
